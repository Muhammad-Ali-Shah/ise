...
def preprocess_text(text):
    ...
    text = lemmatize(text) # Lemmatize *before* removing stopwords
...
REPEAT = 10
out_csv_name = f'../{project}_NB_lemmatized.csv'

data = pd_tplusb
text_col = 'text'

# --- APPLY PREPROCESSING ---
data[text_col] = data[text_col].apply(preprocess_text)  # Apply the combined function
...
for repeated_time in range(REPEAT):
    ...
    X_train = X_all[train_index].toarray()
    X_test = X_all[test_index].toarray()
...
print("=== Gaussian Naive Bayes + TF-IDF (Lemmatized) Results ===")
